 ...
 *  microglia cell volumes, 
 *  lysosomes volumes inside microglia
 *  fibers volume & fibres volume inside lysosomes inside microglia
*/
...
//delete first and last two slices to avoid staining artefacts on slice surface

	getVoxelSize(width, height, depth, unit);
	
	end = (nSlices/3)-2;
	run("Make Substack...", "channels=1-3 slices=3-"+end+"");
...
// Identify fibers
	...
	run("Gaussian Blur 3D...", "x=0.5 y=0.5 z=0.5");
	setAutoThreshold("Li dark stack");//threshold for the histogram of the whole stack
	run("Convert to Mask", "method=Li background=Dark");
	rename("FibersMask");


// Identify lysosomes inside glia
	// use morpho reconstruction to grow lysosomes to full size as sometimes glial stain incompletely encompasses lysosome stain
	// or try 3D spot segmentation

	//get dimensions to set new image size & units for scaling new image
	selectWindow("Orig");
	x = getWidth(); 
	y = getHeight();
	getVoxelSize(width, height, depth, unit);
	...
	n=nSlices;
	newImage("LysoMorphRecon", "8-bit white", x, y, n); //set image name & dimensions
...
//Run the 3Dmanager to 3D segement the Fibers & save the ROIs
		selectWindow("FibersMask");
		run("3D Manager");
		//Ext.Manager3D_Segment(128, 255);
		Ext.Manager3D_AddImage();
		Ext.Manager3D_Measure(); //measure ROIs
		...
		Ext.Manager3D_Delete();
		Ext.Manager3D_Close();
		Ext.Manager3D_SaveResult("M", dir2+ImageSet+"-FibresResults3D.txt"); // save the results
		Ext.Manager3D_CloseResult("M");
		run("Collect Garbage");
